(function ($) {
    ...
    $(document).on("keyup", "#search-inpute", function () {
        let inputVale = $(this).val().trim();
        $("#searchliste li").slice(0).remove();
        $.ajax({
            method: "get",
            url: "product/Searche?search=" + inputVale,
            success: function (res) {
                $("#searchliste").append(res);
            }
        })
    })
    //search backend
